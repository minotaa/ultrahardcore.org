...
<div class="container pl-8">
  ...
  <div class="flex flex-col">
    {#if errors.length > 0}
      <h2 class="text-center text-md bg-red-100 dark:text-white dark:bg-red-500 shadow rounded-lg pt-2 pb-2 pr-8 pl-8 mb-4"><strong>Error:</strong> {errors.join(', ')}</h2>
    {/if}
    <div class="flex-row gap-4">
      ...
      <div class="flex flex-row gap-4">
        <div class="flex flex-col border-slate-200 border-2 dark:border-slate-900 dark:bg-slate-800 bg-slate-200 rounded-lg p-2">
          {#each thisMonth as week}
            <div class="flex flex-row">
              {#each week as day} 
                {#if day == 0}
                  <h2 class="select-none text-lg p-3 bg-slate-100 dark:bg-slate-800 rounded">&nbsp; &nbsp;</h2>
                {:else}
                  <h2 class="select-none text-lg dark:hover:bg-slate-900 border border-slate-700 bg-slate-100 dark:bg-slate-800 dark:text-white p-3">{day}</h2>
                {/if}
              {/each}
            </div>
          {/each}
        </div>
        <div class="flex flex-col border-slate-200 dark:border-slate-900 border-2 dark:bg-slate-800 bg-slate-200 rounded-lg p-2">
          {#each nextMonth as week}
            <div class="flex flex-row">
              {#each week as day} 
                {#if day == 0}
                  <h2 class="select-none text-lg border border-slate-700 bg-slate-100 dark:bg-slate-800 dark:text-white pl-3 pr-3">&nbsp;&nbsp;&nbsp;</h2>
                {:else}
                  <div class="select-none dark:hover:bg-slate-900 border border-slate-700 bg-slate-100 dark:bg-slate-800 p-3">
